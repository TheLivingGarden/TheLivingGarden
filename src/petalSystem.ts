...
function randomizePetal(p: PetalState): void {
  ...
  const MIN_R     = 2.5
  const MAX_R     = PETAL_SPAWN_RADIUS
  // Uniform-area distribution — avoids clustering at the centre
  const radius    = Math.sqrt(
    Math.random() * (MAX_R * MAX_R - MIN_R * MIN_R) + MIN_R * MIN_R
  )
  p.pos = {
 ...
 *  If a settle is in progress, stale burst callbacks are silently dropped. */
export function startPetalRain(): void {
  if (petalSettling) return   // settle in progress — stale callbacks are ignored
  const firstLaunch = !petalActive
  petalActive       = true
...
export function petalParticleSystem(dt: number): void {
  ...
  const dtClamped = Math.min(dt, 0.05)   // cap at 50 ms to survive lag spikes
  ...
  for (const p of petalPool) {
    ...
    // ── Pre-spawn delay ───────────────────────────────────────────
    // phaseMs < 0 = still waiting in the stagger queue.
    if (p.phase === 'spawning' && p.phaseMs < 0) {
      if (!petalActive) {
        // Settle called before this petal launched — cancel immediately
        p.phase     = 'idle'
    ...
    if (p.phase === 'falling') {
      ...
      // Gentle world-space sway
      p.pos.x += Math.sin(time * 2   + p.rotY) * 0.01
      p.pos.z += Math.cos(time * 1.5 + p.rotY) * 0.01

      // Spin: damp out near the ground (cubic — near-zero below 20% proximity)
      const groundProximity = Math.max(0, Math.min(1, (p.pos.y - GROUND_HEIGHT) / 2))
      if (groundProximity < 0.2) {
        p.rotSpeed = 0
      } else {
        const damp  = groundProximity * groundProximity * groundProximity
        const spin  = p.rotSpeed * damp
        const delta = Math.min(Math.abs(spin * dtClamped), 0.08) * Math.sign(spin)
        p.rotY += delta
      }
      ...
      if (p.pos.y <= GROUND_HEIGHT) {
        ...
        p.vel      = { x: 0, y: 0, z: 0 }
        p.rotSpeed = 0
        p.rotY    *= 0.3   // snap remaining angle toward zero — no flip on landing
        p.phase    = 'grounded'
        p.phaseMs  = 0
      }
      ...
      tf.position = { x: p.pos.x, y: p.pos.y, z: p.pos.z }
      tf.rotation = { x: 0, y: Math.sin(halfY), z: 0, w: Math.cos(halfY) }
    ...
    if (p.phase === 'grounded') {
      ...
      // Settle rotation flat over first 400 ms on the ground
      if (p.phaseMs > 400) {
        p.rotY = 0
